...
function LoadCompanyList(hide, id) {
    if ($.cookies.get('ParentId') != "A425BB3A48652CDADB") {
        $("#CompanyHide").css("display", "none !important");
        return false;
    }
    $.ajax({
        type: 'post',
        ...
        success: function(back) {
            ...
            for (var i = 0; i < back.length; i++) {
                html += ' <option value = ' + back[i].CompanyId + '>' + back[i].CompanyName + '</option>';
            }
            $('#' + id + '').append(html);
            return html;
        },
